...
#define DBGOUT(args...)  //pr_debug(args)

/* Clock and Power for PLL */
//memory clock status.

#define MEMCFG			*(volatile unsigned long *)ioremap_nocache(0xc0014800,0x4)
#define MEMTIME0		*(volatile unsigned long *)ioremap_nocache(0xc0014804,0x4)
#define MEMCTRL			*(volatile unsigned long *)ioremap_nocache(0xc0014808,0x4)
#define MEMACTPWD		*(volatile unsigned long *)ioremap_nocache(0xc001480C,0x4)
#define MEMTIME1		*(volatile unsigned long *)ioremap_nocache(0xc0014810,0x4)

#define PHYDELAYCTRL	*(volatile unsigned long *)ioremap_nocache(0xc0014894,0x4)
#define PHYDLLCTRL0		*(volatile unsigned long *)ioremap_nocache(0xc0014898,0x4)

unsigned int cpu_pll1_int;
...
unsigned int cpu_get_clock_hz(int clk);
...
/*	printk("Memory Clock Configuration :\n");
	printk("CLAT =%4d, ALAT =%4d, DLAT =%4d \n", CASLAT, ADDLAT, DLYLAT);
	printk("TRCD =%4d, TRP  =%4d, TRAS =%4d, TMRD =%4d \n", TRCD, TRP , TRAS, TMRD);
	printk("TWR  =%4d, TWTR =%4d, TRTP =%4d, TRFC =%4d \n", TWR , TWTR, TRTP, TRFC);*/
}
...
void pll_init(void)
{
	...
	/* Update CPU PLL only */
	#if (0) /* CTRUE == CFG_SYS_CLKPWR_UPDATE && CTRUE != CFG_SYS_MCUD_UPDATE */
	{
		/* Bus mode */
	...
	#if (1) /* CTRUE == CFG_SYS_CLKPWR_UPDATE && CTRUE == CFG_SYS_MCUD_UPDATE */
	{
		...
		{
			if (1)/* CFG_SYS_CPU_CORE_FREQ != pre_fclk */
			{
				NX_CLKPWR_SetClockCPU(CFG_SYS_CPU_CLKSRC, CFG_SYS_CPU_COREDIV, CFG_SYS_CPU_BUSDIV);
				NX_CLKPWR_SetClockMCLK(CFG_SYS_MCLK_CLKSRC, CFG_SYS_MCLK_DIV, CFG_SYS_BCLK_DIV, CFG_SYS_PCLK_DIV);
				NX_CLKPWR_SetPLLPMS(0, CFG_SYS_PLL0_P, CFG_SYS_PLL0_M, CFG_SYS_PLL0_S);
...
static void cpu_clk_info(void)
{
	...
	NX_CLKPWR_REGSET *pReg = (NX_CLKPWR_REGSET *)ioremap_nocache(NX_CLKPWR_BASEADDR,0x80);
...
//	DBGOUT("%s: %s Frequency Configuration : \n", CFG_SYS_CPU_NAME, (CFG_SYS_CLKPWR_UPDATE==CTRUE?"DONE":"NONE"));
	...
	// ///cpu_pll1_int = pll[1]; // ksw : I want to know all other peripheral now knows CPU pclk clock.
	//printk("PLL1 MHz=%dHz\n", cpu_pll1_int);
